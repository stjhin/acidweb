...
findExpertise('design').css('height',refHeight);findExpertise('development').css('height',refHeight)
}
...
setTimeout(
    function(){$subtitle.text(['From Idea into Product.','User and Data-Driven.','Immersive Experience.',$subtitle.html()].join(',')).Morphext({animation:"fadeInDown",separator:",",speed:4000,complete:function(){}})},4000)}
function mountDocumentEventHandlers(){function toggleNavbarBurguer(){var $burguer=$('.navbar-burger:visible');if($burguer.length){$burguer.toggleClass('is-active');findByID($burguer.data('target')).toggleClass('is-active')}}
$(document).on('click','[data-anchor]',function(e){e.preventDefault();
    var $this=$(this);$('.navbar-item').removeClass('is-active');
    if(!(new RegExp('false','i')).test($this.data('active'))){$this.addClass('is-active')}
Jump(findByID($this.data('anchor'))[0],{callback:toggleNavbarBurguer})})$(document).on('click','.navbar-burger',function(e){e.preventDefault();toggleNavbarBurguer()})}
function mountWindowEventHandlers(){resizeExpertiseCards();$(window).on('resize',resizeExpertiseCards)}$(function(){mountWindowEventHandlers();mountDocumentAnimations();mountDocumentEventHandlers()})